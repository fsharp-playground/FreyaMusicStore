...
type Project() = 
    member __.Configuration(appBuilder : Owin.IAppBuilder) = 
        let musicStore = 
            freyaRouter { 
                resource (UriTemplate.Parse Jw.Routes.home) Jw.Home.pipe
                ...
                resource (UriTemplate.Parse Uris.checkout) Checkout.pipe
            }
            |> FreyaRouter.toPipeline
        appBuilder.Use
            (typeof<CookieAuthenticationMiddleware>, appBuilder, 
        ...
        Microsoft.Owin.Extensions.IntegratedPipelineExtensions.UseStageMarker
            (appBuilder, Owin.PipelineStage.Authenticate) |> ignore
